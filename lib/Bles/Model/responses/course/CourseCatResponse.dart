...
class CourseCatResponse {
  List<CourseCategory> data;
  dynamic message;
  int code;

  static CourseCatResponse fromMap(Map<String, dynamic> map) {
    if (map == null) return null;
    CourseCatResponse CourseLandingResponseBean = CourseCatResponse();
    CourseLandingResponseBean.data = List()..addAll(
      (map['data'] as List ?? []).map((o) => CourseCategory.fromMap(o))
    );
    CourseLandingResponseBean.message = map['message'];
    CourseLandingResponseBean.code = map['code'];
    return CourseLandingResponseBean;
  }
