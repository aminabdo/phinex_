...
class AdminChatBloc extends BaseBloc {
  ...
  getMessagesChat(BaseRequestSkipTake request) async {
    ...
    if(request.skip == 0){
      ...
      _messageChatResponse.value = MessageByChatResponse.fromMap((await repository.get(ApiRoutes.adminChatMessages(request))).data);
  ...
  Future<int> intiateNewChat() async {
    loading.value = true;

    print(repository.post(ApiRoutes.initiateNewChatWithAdmin(), null));

    _chatResponse.value = IntiateNewChatResponse.fromMap(await (await repository.post(ApiRoutes.initiateNewChatWithAdmin(), null)).data);
    ...
    if(_singleChat.value != null) {
      ...
      _singleChat.value = _singleChat.value;
      await initPusher(_singleChat.value.id);
  ...
  addNewMessage(NewMessageRequest request) async {
    ...
    Response response = await repository.postUpload(ApiRoutes.sendMessageToAdmin(request.chat_id),await request.toJson());
  ...
  String channelBaseName = "support-chat.";
  String eventName = "App\\Events\\SupportChatNewMessage";
  String cluster = "eu";
  ...
  Future<void> initPusher(int chatId) async {
    try {
      await Pusher.init(
        ...
        PusherOptions(
          auth: PusherAuth(
              'https://developers.api.phinex.net/v-1872020/eg-en/my/support/$chatId/auth/presence'),
          cluster: cluster,
          host: host,
          port: port,
          encrypted: encrypted,
...
var adminChatBloc = AdminChatBloc();
