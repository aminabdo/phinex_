...
class SingleClinicDetailsPage extends StatefulWidget {
  final int id;
  final String title;

  const SingleClinicDetailsPage(
      {Key key, @required this.id, @required this.title})
      : super(key: key);

  @override
  _SingleClinicDetailsPageState createState() =>
      _SingleClinicDetailsPageState();
}

class _SingleClinicDetailsPageState extends State<SingleClinicDetailsPage> {
  @override
  void initState() {
    super.initState();

    commonBloc.book.onListen = getData();
    commonBloc.getSingle(widget.id, MedicalObjectName.clinic);
  }
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  MyContactsInfoContainer(item.latitude, item.longitude, phone: item.phone, address: item.address, email: item.email, website: item.website),
                  SizedBox(
              ...
              onTap: () async {
                ...
                await commonBloc.bookNow(
                  BookNowRequest(
                    ...
                    clinicId: snapshot.data.data.clinics[index].id.toString(),
                    doctorId:
                        snapshot.data.data.clinics[index].doctorId.toString(),
                    notes: '',
  ...
  void getDateDay() {
    ...
    print(dateDays.toString());
  }

  void Function() getData() {
    commonBloc.book.stream.listen(
      (value) async {
