...
import 'package:phinex/ui/views/auctions/create_aution_lang_page.dart';
...
class _AddLotPageState extends State<AddLotPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: myAppBar(translate(context, 'add_lot'), context),
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: langs.length >= 2 ? null : () async {
                              List<LangAuction> addedLangs = await Navigator.push(
  ...
  void validateAndCreateAuction(BuildContext context) async {
    ...
    if (titleErrorMsg == null &&
      ...
      var response = await highestPriceBloc.createWholesale(auctionRequest);
      if (response.statusCode >= 200 && response.statusCode < 300) {
        AppUtils.showToast(
          msg: translate(context, 'lot_created_successfully'),
          bgColor: mainColor,
        );
        isLoading = false;
        setState(() {});
