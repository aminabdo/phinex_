...
import 'lot_user_view_page.dart';

class LotCategoryPage extends StatefulWidget {
  static final int pageIndex = 0;
  final int id;
  final String name;

  const LotCategoryPage({Key key, @required this.id, this.name})
      : super(key: key);

  @override
  _LotCategoryPageState createState() => _LotCategoryPageState();
}

class _LotCategoryPageState extends State<LotCategoryPage> {
  ...
  void initState() {
    ...
    highestPriceBloc.getAuctionByCat(
  ...
  pusher() async {
    ...
    await highestPriceBloc.subscribeCat();
    await highestPriceBloc.bindCat();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<AuctionsByCatResponse>(
        stream: highestPriceBloc.auctionByCat.stream,
