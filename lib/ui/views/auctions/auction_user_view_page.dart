...
class AuctionUserViewPage extends StatefulWidget {
  final int id;
  final String name;

  const AuctionUserViewPage({Key key, @required this.id, @required this.name})
      : super(key: key);

  @override
  _AuctionUserViewPageState createState() => _AuctionUserViewPageState();
}

class _AuctionUserViewPageState extends State<AuctionUserViewPage> {
  bool readMore = false;
  int currentItem = 0;
  ...
  void initState() {
    ...
    auctionBloc.getAuctionSingle(BaseRequestSkipTake(
      id: widget.id,
      skip: skip,
      take: take,
    ));
  ...
  pusher() async {
    await auctionBloc.initPusherForSingleAuction(widget.id.toString());
    await auctionBloc.connect();
    await auctionBloc.subscribeSingle(widget.id);
    await auctionBloc.bindSingle();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<AuctionSingleResponse>(
        stream: auctionBloc.auctionSingle.stream,
                            ...
                            children: List.generate(
                              ...
                              (index) => AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  color: currentItem == index ? mainColor : mainColor.withOpacity(.3),
                                ),
                              ),
                            ).toList(),
                          ),
                        ...
                        child: Card(
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(AppLocalization.of(context).translate('about_auction'),
                                  style: TextStyle(
                                ...
                                snapshot.data.data.description.length > 70
                                    ? GestureDetector(
                                        child: Text(
                                          readMore ? AppLocalization.of(context).translate('read_less') : AppLocalization.of(context).translate('read_more'),
                                          style: TextStyle(
                                            fontSize: 14,
                                            color: deepBlueColor,
                                          ),
                              ...
                              child: Card(
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        AppLocalization.of(context).translate('auction_has_been_ended'),
                                        style: TextStyle(
                                          fontWeight: FontWeight.bold,
                                          color: Colors.red,
                                          fontSize: 18,
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      MyTextFormField(
                                        hintText: AppLocalization.of(context).translate('place_offer'),
                                      ...
                                      makeAction
                                          ...
                                          : myButton(
                                              ...
                                              onTap: () async {
                                                ...
                                                if (placeAnOffer) {
                                                  if (num.parse(textEditingController.text) > snapshot.data.data.openPrice) {
                                                    if (num.parse(textEditingController.text) > snapshot.data.data.incrementValue) {
                                                      makeAction = true;
                                                      setState(() {});
                                                      await auctionBloc.submitBid(
                                                        SubmitBidRequest(
                                                          userId: AppUtils.userData.id,
                                                          auctionId: 25,
                                                          paidPrice: int.parse(textEditingController.text),
                                                        ),
                                                      );
                                                      AppUtils.showToast(msg: translate(context, 'done'), bgColor: mainColor);
                                                      auctionBloc.auctionSingle.value.data.openPrice = int.parse(textEditingController.text);
                                                    } else {
                                                      AppUtils.showToast(msg: '${translate(context, 'increment_value_is')} ${snapshot.data.data.openPrice}');
                                                    }
                                                  } else {
                                                    AppUtils.showToast(msg: '${translate(context, 'min_price_is')} ${snapshot.data.data.openPrice}');
                                                  }
                                                } else {
                                                  if (num.parse(textEditingController.text) > snapshot.data.data.openPrice) {
                                                    makeAction = true;
                                                    setState(() {});
                                                    await auctionBloc.makeDeal(
                                                      MakeDealRequest(
                                                        auctionId: widget.id,
                                                        userId: AppUtils.userData.id,
                                                        price: double.parse(textEditingController.text).toInt(),
                                                      ),
                                                    );
